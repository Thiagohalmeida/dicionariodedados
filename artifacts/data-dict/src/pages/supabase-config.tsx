...
export default function SupabaseConfigPage() {
    ...
    try {
      ...
      if (res.ok) {
        toast({
          ...
          description: "As configurações do Supabase foram atualizadas.",
        });
        checkStatus();
      } else {
        ...
        toast({
          ...
          description: err.error ?? "Falha ao salvar configuração",
          variant: "destructive",
        });
      }
    } catch (err) {
      toast({
        title: "Erro",
        description: "Erro de conexão ao salvar",
        variant: "destructive",
      });
    } finally {
      setSaving(false);
  ...
  const handleCreateBuckets = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast({
          title: "Buckets criados",
          description: "Buckets de storage foram criados/verificados.",
        });
        checkStatus();
      } else {
        const err = await res.json();
        toast({
          title: "Erro",
          description: err.error ?? "Falha ao criar buckets",
          variant: "destructive",
        });
      }
    } catch (err) {
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <div>
        ...
        <p className="text-muted-foreground">
          Configure a conexão com o Supabase para Storage, Auth e validação DDL
          em tempo real.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Verifique se o Supabase está acessível e os buckets estão
            configurados.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Conexão com o banco PostgreSQL e Storage
              </p>
            </div>
            <ConnectionStatus />
          </div>
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              {creatingBuckets ? "Criando..." : "Criar/Verificar Buckets"}
            </Button>
          </div>
        </CardContent>
      </Card>
        ...
        <TabsContent value="config" className="mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                URL do projeto, chave anônima e chave de serviço. A chave de
                serviço <strong>nunca</strong> é exposta no frontend.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Usada para operações do lado do cliente (futuro)
                </p>
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setConfig({ ...config, serviceRoleKey: e.target.value })
                  }
                  type="password"
                />
                <p className="text-xs text-muted-foreground">
                  Usada apenas no backend para operações admin.{" "}
                  <strong>Nunca compartilhe.</strong>
                </p>
              </div>
        ...
        <TabsContent value="storage" className="mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Configure os buckets para upload de Excel e exportação de
                DDL/Data Contract.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2 p-4 border rounded-lg">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setConfig({ ...config, bucketExcel: e.target.value })
                    }
                  />
                  <p className="text-xs text-muted-foreground">
                    Arquivos .xlsx/.xlsm enviados pelos usuários
                  </p>
                </div>
                <div className="space-y-2 p-4 border rounded-lg">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setConfig({ ...config, bucketExports: e.target.value })
                    }
                  />
                  <p className="text-xs text-muted-foreground">
                    DDL (.sql), Data Contract (.json), CSV
                  </p>
                </div>
              </div>
                    ...
                    {status.buckets.map((bucket) => (
                      ...
                      >
                        ...
                        <p className="text-xs text-muted-foreground">
                          ...
                          {bucket.fileSizeLimit / 1024 / 1024}MB
                        </p>
                      </div>
                    ))}
                  </div>
        ...
        <TabsContent value="ddl" className="mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Teste a criação de tabelas no PostgreSQL real antes de promover
                para produção.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="p-4 bg-muted/50 rounded-lg">
                ...
                <ol className="list-decimal list-inside space-y-1 text-sm text-muted-foreground">
                  ...
                  <li>
                    O sistema gera o DDL (CREATE TABLE) baseado nos campos
                  </li>
                  <li>
                    Executa o DDL em um schema temporário (
                    {"staging_<id>_<timestamp>"})
                  </li>
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Acesse um dicionário em {"/dictionaries/:id"} e use o endpoint{" "}
                  {"POST /api/dictionaries/:id/validate-ddl"}
                </p>
                <Button
                  variant="outline"
                  className="gap-2"
                  onClick={() => window.open("/dictionaries", "_blank")}
                >
                  Ir para Dicionários
                </Button>
              </div>
