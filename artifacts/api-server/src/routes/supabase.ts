...
import { getSupabaseClient, getSupabaseAdminClient, type BucketName } from "../modules/supabase/client";
...
router.post("/supabase/config", async (req: Request, res: Response): Promise<void> => {
  const { url, anonKey, serviceRoleKey, bucketExcel, bucketExports } = req.body as Partial<typeof supabaseConfig>;

  if (url !== undefined) supabaseConfig.url = url;
  if (anonKey !== undefined) supabaseConfig.anonKey = anonKey;
  if (serviceRoleKey !== undefined) supabaseConfig.serviceRoleKey = serviceRoleKey;
  if (bucketExcel !== undefined) supabaseConfig.bucketExcel = bucketExcel;
  if (bucketExports !== undefined) supabaseConfig.bucketExports = bucketExports;
...
router.get("/supabase/status", async (_req: Request, res: Response): Promise<void> => {
  try {
    if (!supabaseConfig.url || !supabaseConfig.serviceRoleKey) {
      res.json({
        ...
        buckets: [],
        authEnabled: false,
        error: "Supabase não configurado (SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY necessários)",
      });
      return;
    }
    ...
    try {
      const { data: authSettings } = await supabase.auth.admin.listUsers({ page: 1, perPage: 1 });
      authEnabled = !!authSettings;
    } catch {
      authEnabled = false;
    }

    const bucketInfos = buckets?.map((b) => ({
      id: b.id,
      name: b.name,
      public: b.public,
...
router.post("/supabase/buckets", async (_req: Request, res: Response): Promise<void> => {
  try {
    if (!supabaseConfig.url || !supabaseConfig.serviceRoleKey) {
    ...
    res.json({ success: true, message: "Buckets verificados/criados com sucesso" });
  } catch (err) {
    res.status(500).json({ error: err instanceof Error ? err.message : "Erro ao criar buckets" });
  }
});
