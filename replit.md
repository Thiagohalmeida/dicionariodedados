...
- `artifacts/api-server/src/routes/` — Route handlers: `dictionaries.ts`, `fields.ts`, `dashboard.ts`
...
- Export supports JSON and CSV via `?format=csv` query param on `GET /api/dictionaries/:id/export`
...
- **Import**: Paste/upload JSON following the standard schema (processo, categoria, tabela, campos[])
...
- **Export**: Download validated dictionary as JSON or CSV
